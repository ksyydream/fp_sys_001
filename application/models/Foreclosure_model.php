...
class Foreclosure_model extends MY_Model
{
    ...
    //推送待审核通知
    public function wx_f_sf($fc_id){
        $foreclosure_info = $this->db->select('foreclosure_id, work_no, m_id, user_id')->from('foreclosure')->where(array('foreclosure_id' => $fc_id))->get()->row_array();
        if(!$foreclosure_info)
            return -1;
    ...
    public function foreclosure_err_finish($m_info){
        $f_id = $this->input->post('fc_id');
        if(!$f_id)
            return $this->fun_fail('此赎楼业务不存在!');
        $f_info = $this->get_foreclosure($f_id);
        ...
        $this->db->where(array('foreclosure_id' => $f_id, 'status' => 3))->update('foreclosure', $update_);
        return $this->fun_success('终审不通过', array('foreclosure_id' => $f_id));
    }
}